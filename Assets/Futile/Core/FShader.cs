...
public class FClipShader : FShader
{
	private Rect m_tClipRect;
	public FClipShader(Rect tClipRect) : base("BasicClip", Shader.Find("Futile/BasicClip"))
	{
		m_tClipRect = tClipRect;
		needsApply = true;
	}

	override public void Apply(Material mat)
	{
		//Offset these values for the shader
		m_tClipRect.width *= 0.5f;
		m_tClipRect.height *= 0.5f;
		mat.mainTextureOffset = new Vector2(-tClipRect.x / tClipRect.width, -tClipRect.y / tClipRect.height);
		mat.mainTextureScale = new Vector2(1f / tClipRect.width, 1f / tClipRect.height);
	}

	public Rect tClipRect
	{
		get {return m_tClipRect;}
		set {if(m_tClipRect != value) {m_tClipRect = value; needsApply = true;}}
	}
}
