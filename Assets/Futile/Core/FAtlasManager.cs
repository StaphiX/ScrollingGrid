...
public class FAtlasManager
{
	...
	public FAtlasManager () //new FAtlasManager() called by Futile
	{
	...
	{
		string[] sFiles = Directories.GetFileNames(Directories.GetFullTexturePath(), "*.png");
		foreach (string sFileName in sFiles) 
		{
			string sTextureFile = Directories.sFolder + "/" + 
				Directories.GetFileNameFromPath(sFileName, false);
			LoadImage(sTextureFile);
		}
	}
	
	public void LoadAllAtlases()
	{
		string[] sFolders = Directories.GetFolderNames (Directories.GetFullAtlasPath ());
		foreach (string sFolderName in sFolders) 
		{
			string sAtlasFolder = sFolderName + "/" + Directories.sAtlasSubFolder;
			string[] sFiles = Directories.GetFileNames(sAtlasFolder, "*.png");
			foreach (string sFileName in sFiles) 
			{
				string sAtlasFolderName = Directories.sAtlasFolder + "/" +
					Directories.GetFileNameFromPath(sFolderName, false) + "/" + 
						Directories.sAtlasSubFolder;
				string sAtlasFile = sAtlasFolderName + "/" + Directories.GetFileNameFromPath(sFileName, false);
				LoadAtlas(sAtlasFile);
			}
		}
	}
